...
- 又或者，您也可以使用我们 [GitHub 上的模板项目](../../vcc.docs.vrchat.com/guides/using-project-template-repos.md)。如果您不使用创作者助手，还需要下载 [当前版本的 Unity](/creators.vrchat.com/sdk/current-unity-version.md)。我们强烈建议您使用 Unity Hub 下载 Unity，详见[此处](./current-unity-version.md)。
...
对于我们的第一个项目，我们假定您正在为 Windows PC 构建内容。如果您想了解为 Quest 创建 VRChat 内容的过程，请查看 [为 Unity 配置 Quest 内容开发环境](/creators.vrchat.com/platforms/android/setting-up-unity-for-creating-quest-content.md)。

创建预先配置好的项目最简单的方式是使用 VRChat 创作者助手！我们强烈建议使用创作者助手来完成这些工作。否则你将需要手动完成大量额外步骤而且很容易出错。
...
如果愿意，您可以手动创建一个项目，但无论如何，您都需要使用 [Creator Companion](/vcc.docs.vrchat.com/index.md) 来安装 SDK（除非您从我们的[模板仓库](/vcc.docs.vrchat.com/guides/using-project-template-repos)开始安装）。
...
要使用 SDK，您需要登录。在编辑器中点击 `VRChat SDK > Show Control Panel > Authentication`。您可以在此登录您的 VRChat 账号。

请记住，您的 VRChat 账号必须至少为 "新用户 (New User)" 的[信誉等级](/docs.vrchat.com/SYSTEM/vrchat-safety-and-trust-system.md) 才能上传内容。您不能使用 Steam、Oculus 或 Viveport 帐户上传内容。
