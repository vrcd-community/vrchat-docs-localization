...
| [Viseme](#viseme-values)                             | 表示口型，大小范围为`0-14`，参考自 [Oculus 面部表情目录](https://developer.oculus.com/documentation/unity/audio-ovrlipsync-viseme-reference)。当模型的 Viseme 模式使用 Jawbone/Jawflap 时，表示音量，大小范围为 `0-100`。 <!--暂时换回Viseme，等待更合理的解释-->| 整数         | Viseme           |
| Voice                                                | 当前麦克风音量（`0.0-1.0`）                                                                                                                                                          | 浮点数       | Viseme           |
...
| GestureLeftWeight                                    | 左手的扳机按压幅度（0.0-1.0）†                                                                                                                                                     | 浮点数       | 可播放层       |
| GestureRightWeight                                   | 右手的扳机按压幅度（0.0-1.0）†                                                                                                                                                     | 浮点数       | 可播放层       |
...
| Expression1 - Expression16                           | 玩家自定义的参数，可以是整数（`0`-`255`）或浮点数（`-1.0`-`1.0`）                                                                                                                         | 整数 / 浮点数 | IK 或可播放层 |
| [TrackingType](#trackingtype-parameter)              | 请参阅下文描述                                                                                                                                                                  | 整数         | 可播放层       |
| VRMode                                               | 如果玩家处于 VR 模式中，则返回 `1`；如果不在 VR 模式中，则返回 `0`                                                                                                                        | 整数         | IK             |
| MuteSelf                                             | 如果玩家已将自己静音，则返回 `true`；如果未静音，则返回 `false`                                                                                                                     | 布尔        | 可播放层       |
| InStation                                            | 如果玩家在 VRCStation 中，则返回 `true`；如果不在站点中，则返回 `false`                                                                                                                     | 布尔        | IK             |
| Earmuffs                                             | 如果玩家已打开耳罩功能，则返回 `true`；如果已关闭，则返回 `false`                                                                                                                     | 布尔        | 可播放层       |
...
| 名称               | 描述                                                                                                                                                                                                 | 类型  | 同步     |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------- |
| ScaleModified      | 如果玩家缩放了模型，则返回 `true`；如果模型处于默认大小，则返回 `false`                                                                                                                     | 布尔  | 可播放层 |
| ScaleFactor        | 模型当前高度与默认高度之间的比例。例如当默认眼高为 1 米的模型缩放到 2 米时，将表现为`2`                                                                                                                        | 浮点数 | 可播放层 |
| ScaleFactorInverse | 模型当前高度与默认高度之间的倒数比例（`1/x`）。例如当默认眼高为 1 米的模型缩放到 2 米时，将表现为 `0.5`。该值在极端情况下可能不准确。                                                                                   | 浮点数 | 可播放层 |
| EyeHeightAsMeters  | 模型的眼高（以米为单位），例如模型的眼高为`1.5`。                                                                                                                                                                             | 浮点数 | 可播放层 |
| EyeHeightAsPercent | 模型的眼高与默认缩放限制（`0.2`-`5.0`）之间的关系。缩放为 2 米的模型将报告 `(2.0 - 0.2) / (5.0 - 0.2)` = `0.375`                                                                                     | 浮点数 | 可播放层 |
...
| 3    | 头部和手部追踪，如果 `VRMode` 为 `1`，则用户处于 3 点 VR 中。如果 `VRMode` 为 `0`，则这是一个使用人形模型的桌面用户。                                                                                                                   |
