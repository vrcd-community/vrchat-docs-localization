...
`VRCParentConstraint` 组件移动和旋转目标变换，就像它是其目标对象的子对象一样。

![img](https://cn-nb1.rains3.com/docs-image/controls/vrc-parent-constraint-1.png)
...
- **静止时的位置/旋转（Position/Rotation At Rest）**- 定义约束整体权重为 0 时变换的位置/旋转。

- **位置/旋转偏移量（Position/Rotation Offset）**- 定义应用于此约束结果的偏移量。
...
- **冻结位置/旋转轴（Freeze Position/Rotation Axes）**- 允许您在解决约束时排除特定轴的计算。默认情况下，所有三个轴都被选中进行计算。

::: info 信息
在使用父约束时，每个源的偏移量都是单独设置的。:::
