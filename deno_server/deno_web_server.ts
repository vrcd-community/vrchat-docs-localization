...
import { join as posixJoin } from "$std/path/posix/join.ts"
import { normalize as posixNormalize } from "$std/path/posix/normalize.ts"
import { extname } from "$std/path/extname.ts"
import { join } from "$std/path/join.ts"
import { relative } from "$std/path/relative.ts"
import { resolve } from "$std/path/resolve.ts"
import { SEPARATOR_PATTERN } from "$std/path/constants.ts"
...
async function lookupFile(filePath: string): Promise<FileLookupResult | null> {
  const filePaths = [filePath, filePath + ".html"]
...
async function createServeDirResponse(req: Request, opts: ServeDirOptions) {
  ...
  const FileLookupResult = await lookupFile(fsPath)

  if (!FileLookupResult) {
    return createStandardResponse(STATUS_CODE.NotFound)
  }

  const { fileInfo, filePath } = FileLookupResult
  ...
  if (showDirListing) {
    ...
    return serveDirIndex(filePath, { urlRoot, showDotfiles, target, quiet })
  }
