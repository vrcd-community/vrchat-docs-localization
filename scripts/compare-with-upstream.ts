...
for (const [docsPath, docItems] of Object.entries(result)) {
  ...
  tableItems.push(...docItems.map(item => [
    ...
    { data: item.upstreamPath ? `<code><a href="${getGithubLink(item.upstreamPath, item.latestUpstreamCommit ?? 'main', 'blob', upstreamRepo)}">${item.upstreamPath}</a></code>` : '<code>none</code>' },
